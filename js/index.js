...
$(function(){
    ...
    for(let i = 1 ; i <=  12 ; i++){
        ...
        optionWeek.text = i
        optionSat.text = i
        optionWeek.innerHTML = i
        optionSat.innerHTML = i

       fillHourDropDown(optionWeek,optionSat) 
    }


})

function fillHourDropDown(...opt){

    const [optionWeek,optionSat] = opt

    hourWeek.appendChild(optionWeek)
    hourSaturday.appendChild(optionSat)
...
function setElements(event,id,nextElement){
    ...
    event.addEventListener("keyup", (event) => {
        ...
        let el = currentCell.value
        
// *********

        if(el.length >= 4){
           
            if(el.substring(0,1) < 10 && el.substring(0,1) > 2){
               
                let currentData = `0${el.substring(0,1)}:${el.substring(1,2)}${el.substring(3)}`
